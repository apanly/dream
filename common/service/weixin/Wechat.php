...
class Wechat {
    ...
    private $userAgent = 'Mozilla/5.0 (Macintosh; Intel Mac OS X 10_10_2) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/45.0.2454.101 Safari/537.36
';
    ...
    private function curlPost($url) {
        ...
        curl_setopt($curl, CURLOPT_HTTPHEADER, array('Expect:')); //设置HTTP头字段的数组
        curl_setopt($curl, CURLOPT_SSL_VERIFYPEER, 0); //对认证证书来源的检查
        curl_setopt($curl, CURLOPT_SSLVERSION, 4);
        curl_setopt($curl, CURLOPT_SSL_VERIFYHOST, 3); //从证书中检查SSL加密算法是否存在
